...
for good in (good1,good2):
	...
	legend(loc=2)
	#title('Signal to Noise over a '+str(r)+'" radius aperture on the '+thing[i])
...
#mx=arange(0.9,1.35,(1.35-.9)/len(mspec))
	...
	tick_params(axis='x',labelbottom='off')
	locator_params(axis='y',prune='lower')
	#title('Mean spectrum over a '+str(r)+'" radius appetrure on the '+thing[i])
	...
	legend()
	#title('INT_SCI*NFRAMES_SCIContribution of Various Noise Sources')
